...
def umap_projection(output_dir: str, data_dir: str = "1000G", n_pcs: int = 0) -> None:
    ...
    n_pcs_auto, _ = marchenko_pastur_pc_count_from_data_dir(
        data_dir, eigenvalues, max_pcs=max_pcs
    )

    pc_cols = [f"PC{i}" for i in range(1, n_pcs_auto + 1)]
    ...
    for ax, (col, pal) in zip(axes, overlays):
        ...
        ax.set_title(f"UMAP ({n_pcs_auto} PCs, MP) — {col}")
        ax.legend(markerscale=3, fontsize=7, loc="best")

    fig.tight_layout()
    out_path = os.path.join(output_dir, f"umap_{n_pcs_auto}pcs.png")
    fig.savefig(out_path, dpi=200, bbox_inches="tight")
    plt.close(fig)
    print(f"[03] UMAP projection ({n_pcs_auto} PCs, MP) → {out_path}")
...
def main() -> None:
    ...
    parser.add_argument("--n-pcs", type=int, default=0,
                        help="Optional upper bound for MP-selected PCs (0 = no cap)")
    args = parser.parse_args()
    umap_projection(args.output_dir, args.data_dir, args.n_pcs)
