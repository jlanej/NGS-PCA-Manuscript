...
def _load_full_merged_from_data_dir(data_dir: str) -> pd.DataFrame:
    ...
    # sample_qc.tsv (NGS-PCA ≥ PR #34) already provides correct SUPERPOPULATION
    # and FAMILY_ROLE columns — no remapping needed.
...
def _load_within_ancestry_batch(output_dir: str):
    """Load within-ancestry batch results from the TSV produced by 09_within_ancestry_batch.py.

    Returns a dict with keys ``pc_cols``, ``superpops``, and ``results`` (nested
    dict superpop → variable → pc → {observed_eta2, p_value, n_samples}) or None
    ...
    pc_cols = sorted(df["PC"].unique().tolist(), key=lambda p: int(p[2:]))
    superpops = [g for g in ["AFR", "AMR", "EAS", "EUR", "SAS"]
                 if g in df["Superpopulation"].values]

    results: dict = {}
    for sp in superpops:
        results[sp] = {}
        sub_sp = df[df["Superpopulation"] == sp]
        for var in ["RELEASE_BATCH", "FAMILY_ROLE"]:
            sub_v = sub_sp[sub_sp["Variable"] == var]
            if sub_v.empty:
                continue
            n_samples = int(sub_v["n_samples"].iloc[0]) if "n_samples" in sub_v.columns else 0
            has_perm = "p_value" in sub_v.columns and sub_v["p_value"].notna().any()
            n_perm = int(sub_v["n_permutations"].iloc[0]) if "n_permutations" in sub_v.columns else 0
            results[sp][var] = {
                "n_samples": n_samples,
                ...
                "per_pc": {
                    row["PC"]: {
                        ...
                        if not pd.isna(row["observed_eta2"]) else 0.0,
                        "p_value": float(row["p_value"])
                        if has_perm and not pd.isna(row["p_value"]) else None,
                    }
...
def _load_robust_qc_variance(output_dir: str):
    """Load QC-metric variance partitioning from 12_robust_qc_variance.py.

    Returns a list of row dicts (one per QC metric) or None if the file is
    absent.
    """
    ...
    for _, row in df.iterrows():
        records.append({
            ...
            "unique_ancestry": float(row["unique_ancestry"]),
            "unique_batch": float(row["unique_batch"]),
            "shared": float(row["shared"]),
...
def _build_html(
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    <div class="report-section" id="section-within-ancestry">
      <h2>Within-Ancestry Stratified Batch Effect</h2>
      <div class="description">
        <h3>Rationale</h3>
        <p>
          The strongest possible evidence that NGS-PCA captures <strong>batch</strong> —
          not ancestry — is to show that <em>within a single superpopulation</em>
          (e.g., EUR-only samples), batch still drives PC separation.  If batch η²
          remains large within each ancestry group, the batch effect is independent
          of population structure.  If it vanishes, the "batch" signal was confounded
          with ancestry.
        </p>
        <h3>Method</h3>
        <p>
          ...
          exact <em>p</em>-values (Phipson &amp; Smyth conservative correction).
          Results are generated by <code>scripts/09_within_ancestry_batch.py</code>.
        </p>
        <h3>Results</h3>
    ...
    <div class="report-section" id="section-refbias">
      ...
      <div class="description" style="margin-top:2rem;">
        <h3>Variance Partitioning of QC Coverage Metrics</h3>
        <p>
          <strong>Rationale.</strong>&ensp;The partial η² regression above tests
          whether each QC metric differs significantly across ancestries after
          adjusting for batch.  Here we go further and decompose the <em>total
          explained variance</em> (R²) of each QC metric into three components:
        </p>
        <ul>
          <li><strong>Unique ancestry</strong> — R²<sub>full</sub> − R²<sub>batch only</sub>:
              variance uniquely attributable to ancestry (SUPERPOPULATION) after
              controlling for batch.</li>
          <li><strong>Unique batch</strong> — R²<sub>full</sub> − R²<sub>ancestry only</sub>:
              variance uniquely attributable to batch (RELEASE_BATCH) after
              controlling for ancestry.</li>
          <li><strong>Shared / confounded</strong> — variance explained by both
              predictors jointly, which cannot be unambiguously assigned.
              Large shared components indicate that ancestry and batch are
              collinear for that metric.</li>
        </ul>
        <p>
          If the reference genome introduces ancestry-correlated coverage bias,
          the <em>unique ancestry</em> component should be non-negligible for
          coverage metrics like mean/median, while dispersion metrics (MAD, IQR)
          may be less affected.  Conversely, if observed ancestry–coverage
          associations vanish after conditioning on batch, the signal is driven
          by batch composition rather than reference-genome bias.
        </p>
        <p>
          Results are generated by <code>scripts/12_robust_qc_variance.py</code>
      ...
      <div class="plot-card" style="margin-top:1.5rem;">
        <div class="plot-card-header">
          <h3>QC Metric Variance Decomposition — Ancestry vs Batch</h3>
        </div>
        <div class="plot-card-body">
          <div id="rqv-stacked-bar" style="height:400px"></div>
        </div>
      </div>

      <div class="plot-card" style="margin-top:1.5rem;">
        <div class="plot-card-header">
          <h3>QC Metrics by Superpopulation (Batch-Conditioned)</h3>
          <select id="rqv-batch-metric-select" style="margin-left:1rem;padding:0.25rem 0.5rem;border-radius:4px;border:1px solid #cbd5e1;font-size:0.85rem;"></select>
        </div>
        <div class="plot-card-body">
          <div id="rqv-batch-violin" style="height:450px"></div>
        </div>
      </div>
    </div>
    ...
    /* ------------------------------------------------------------------ */
    (function() {
      var wa = DATA.within_ancestry;
      var summaryEl = document.getElementById('within-ancestry-summary');
      var facetsEl  = document.getElementById('within-ancestry-facets');
      if (!wa || !wa.superpops || wa.superpops.length === 0) {
        summaryEl.textContent =
          'Within-ancestry batch results not yet available. Run scripts/09_within_ancestry_batch.py first.';
        return;
      }

      var pcs      = wa.pc_cols;
      var superpops = wa.superpops;
      var vars = ['RELEASE_BATCH', 'FAMILY_ROLE'];
      var varLabels = {'RELEASE_BATCH': 'Batch (\u03b7\u00b2)', 'FAMILY_ROLE': 'Family Role (\u03b7\u00b2)'};
      var varColors = {'RELEASE_BATCH': '#D95F02', 'FAMILY_ROLE': '#7570B3'};
      ...
      /* Summary paragraph */
      var nSigBatch = 0, nTotalBatch = 0;
      superpops.forEach(function(sp) {
        vars.forEach(function(v) {
          var vdata = wa.results[sp] && wa.results[sp][v];
          if (!vdata) return;
          pcs.forEach(function(pc) {
            var r = vdata.per_pc && vdata.per_pc[pc];
            if (!r) return;
            if (v === 'RELEASE_BATCH') {
              nTotalBatch++;
              if (r.p_value !== null && r.p_value < 0.05) nSigBatch++;
            }
          });
        });
      });
      var nSamples = superpops.map(function(sp) {
        var vd = wa.results[sp] && (wa.results[sp]['RELEASE_BATCH'] || wa.results[sp]['FAMILY_ROLE']);
        return sp + '\u202f=\u202f' + (vd ? vd.n_samples : '?');
      }).join(', ');
      summaryEl.innerHTML =
        'Within-ancestry stratified batch η² across <strong>' + superpops.length
        + '</strong> superpopulations (' + nSamples + '). '
        + 'Batch (RELEASE_BATCH) is significant (p\u202f&lt;\u202f0.05) in '
        + '<strong>' + nSigBatch + ' / ' + nTotalBatch + '</strong> PC × group combinations, '
        + 'indicating that batch effects persist <em>within</em> each ancestry group.';

      /* One Plotly bar chart per superpopulation, laid out in a flex row */
      var rowDiv = document.createElement('div');
      rowDiv.style.cssText = 'display:flex;flex-wrap:wrap;gap:1.5rem;margin-top:1rem;';
      facetsEl.appendChild(rowDiv);

      superpops.forEach(function(sp) {
        var card = document.createElement('div');
        card.className = 'plot-card';
        card.style.cssText = 'flex:1;min-width:260px;max-width:480px;';
        card.innerHTML =
          '<div class="plot-card-header"><h3>' + sp + '</h3></div>'
          + '<div class="plot-card-body"><div id="wa-plot-' + sp + '" style="height:320px"></div></div>';
        ...
        var traces = vars.map(function(v) {
          ...
          var ys = [], xs = [], texts = [], hovers = [];
          pcs.forEach(function(pc) {
            var r = vdata.per_pc && vdata.per_pc[pc];
            var eta2 = r ? r.observed_eta2 : 0;
            var pv   = r ? r.p_value : null;
            xs.push(pc);
            ys.push(eta2);
            texts.push(sigLabel(pv));
            hovers.push(varLabels[v] + '<br>' + sp + ' \u00d7 ' + pc
              + '<br>\u03b7\u00b2 = ' + eta2.toFixed(4)
              + (pv !== null ? '<br>p = ' + (pv < 0.001 ? pv.toExponential(2) : pv.toFixed(4))
                             : '')
              + (sigLabel(pv) ? '<br>' + sigLabel(pv) : ''));
          });
          return {
            x: xs, y: ys, type: 'bar', name: varLabels[v],
            marker: {color: varColors[v]},
            text: texts, textposition: 'outside', textfont: {size: 8},
            hovertext: hovers, hoverinfo: 'text',
          };
        }).filter(Boolean);

        Plotly.newPlot('wa-plot-' + sp, traces, {
          ...LAYOUT_BASE,
          barmode: 'group',
          xaxis: {...LAYOUT_BASE.xaxis, title: 'PC', tickangle: -45, tickfont: {size: 9}},
          yaxis: {...LAYOUT_BASE.yaxis, title: '\u03b7\u00b2'},
          showlegend: true,
          legend: {x: 0.01, y: 0.99, bgcolor: 'rgba(255,255,255,0.8)',
                   bordercolor: '#cbd5e1', borderwidth: 1, font: {size: 9}},
          margin: {...LAYOUT_BASE.margin, b: 70, t: 30},
        }, CFG);
      });
    })();
    ...
    /* ------------------------------------------------------------------ */
    (function() {
      var rqv = DATA.robust_qc_variance;
      var qc = DATA.reference_bias_qc;
      var summaryEl = document.getElementById('rqv-summary');
      if (!rqv || rqv.length === 0) {
        if (summaryEl) summaryEl.textContent =
          'QC variance partitioning results not yet available. Run scripts/12_robust_qc_variance.py first.';
        return;
      }

      /* ---- Summary paragraph ---- */
      var meanAnc = rqv.reduce(function(s,r){return s+r.unique_ancestry;},0) / rqv.length;
      var meanBatch = rqv.reduce(function(s,r){return s+r.unique_batch;},0) / rqv.length;
      var maxAnc = rqv.reduce(function(mx,r){return Math.max(mx,r.unique_ancestry);},0);
      var maxBatch = rqv.reduce(function(mx,r){return Math.max(mx,r.unique_batch);},0);
      var nAncDom = rqv.filter(function(r){return r.unique_ancestry > r.unique_batch;}).length;
      summaryEl.innerHTML =
        'Leave-one-out R\u00b2 decomposition of <strong>' + rqv.length
        + '</strong> QC coverage metrics. '
        + 'Mean unique ancestry R\u00b2\u202f=\u202f' + meanAnc.toFixed(4)
        + ', mean unique batch R\u00b2\u202f=\u202f' + meanBatch.toFixed(4) + '. '
        + '<strong>' + nAncDom + '/' + rqv.length
        + '</strong> metrics have a larger unique ancestry than unique batch component. '
        + (maxAnc < 0.01
           ? 'The negligible unique ancestry components indicate that ancestry-correlated '
             + 'coverage variation is <strong>fully confounded with batch</strong> in this dataset.'
           : 'Non-trivial unique ancestry components suggest some reference-genome-driven '
             + 'coverage variation persists after conditioning on batch.');

      /* ---- Stacked bar chart ---- */
      var metrics = rqv.map(function(r){return r.metric;});
      var compOrder = ['unique_batch','unique_ancestry','shared','residual'];
      var compLabels = {
      ...
      var traces = compOrder.map(function(comp) {
        return {
          x: metrics, type: 'bar', name: compLabels[comp],
          marker: {color: compColors[comp]},
          y: rqv.map(function(r){return r[comp];}),
          hovertemplate: '%{x}<br>' + compLabels[comp]
            + ': %{y:.4f}<extra></extra>',
        };
      });
      Plotly.newPlot('rqv-stacked-bar', traces, {
        ...LAYOUT_BASE,
        barmode: 'stack',
        showlegend: true,
        legend: {x: 0.01, y: 0.99, bgcolor: 'rgba(255,255,255,0.8)',
                 bordercolor: '#cbd5e1', borderwidth: 1, font: {size: 10}},
        xaxis: {...LAYOUT_BASE.xaxis, title: 'QC Metric', tickangle: -35, tickfont: {size: 9}},
        yaxis: {...LAYOUT_BASE.yaxis, title: 'Proportion of Total Variance', range: [0, 1.05]},
        margin: {...LAYOUT_BASE.margin, b: 120},
      }, CFG);

      /* ---- Within-batch violin plots ---- */
      var batchMetrics = qc ? Object.keys(qc) : [];
      var batchSel = document.getElementById('rqv-batch-metric-select');
      batchMetrics.forEach(function(m) {
        var opt = document.createElement('option');
        opt.value = m; opt.textContent = m;
        batchSel.appendChild(opt);
      });

      function drawBatchViolin(metric) {
        var d = qc[metric];
        if (!d) return;
        var batches = [...new Set(d.batch)].sort();
        var spops = ['AFR','AMR','EAS','EUR','SAS'];
        var trcs = [];
        batches.forEach(function(b) {
          spops.forEach(function(sp) {
            var vals = [];
            for (var i = 0; i < d.values.length; i++) {
              if (d.batch[i] === b && d.superpop[i] === sp) vals.push(d.values[i]);
            }
            if (vals.length > 0) {
              trcs.push({
                type: 'violin', y: vals,
                ...
                marker: {color: PAL_SUPERPOP[sp]},
                points: 'outliers',
                opacity: b === batches[0] ? 1.0 : 0.6,
              });
            }
          });
        });
        Plotly.react('rqv-batch-violin', trcs, {
          ...LAYOUT_BASE,
          showlegend: true,
          violinmode: 'group',
          yaxis: {...LAYOUT_BASE.yaxis, title: metric},
      ...
      if (batchMetrics.length > 0) drawBatchViolin(batchMetrics[0]);
      batchSel.addEventListener('change', function() { drawBatchViolin(this.value); });
    })();
...
def generate_report(
    ...
    print("[06] Loading within-ancestry batch results …")
    within_ancestry_results = _load_within_ancestry_batch(output_dir)
    ...
    print("[06] Loading robust QC variance partitioning results …")
    robust_qc_variance_results = _load_robust_qc_variance(output_dir)
